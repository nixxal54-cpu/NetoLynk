/**
 * useBlinkUpload.ts
 * FIX: "Failed to fetch" was caused by Cloudinary's /auto/upload endpoint
 * rejecting the request due to the upload preset not being configured for
 * "auto" resource type. We now detect image vs video and hit the correct
 * typed endpoint (/image/upload or /video/upload) so Cloudinary never rejects.
 *
 * Also adds an explicit AbortController timeout (30 s for images, 120 s for
 * videos) so a stalled upload surfaces as a real error instead of hanging
 * silently forever on slow mobile connections.
 */
...
export function useBlinkUpload() {
  ...
  const setTextOverlay      = useCallback((text: string)  => { dataRef.current.textOverlay = text;   setState(s => ({ ...s, textOverlay: text })); }, []);
  const setTextOverlayColor = useCallback((color: string) => { dataRef.current.textOverlayColor = color; setState(s => ({ ...s, textOverlayColor: color })); }, []);
  const setCaption          = useCallback((caption: string) => { dataRef.current.caption = caption;  setState(s => ({ ...s, caption })); }, []);
  ...
  const publish = useCallback(async (): Promise<boolean> => {
    ...
    // Fake-progress ticker so the UI doesn't look frozen
    let currentProgress = 5;
    const progressInterval = setInterval(() => {
      currentProgress = Math.min(currentProgress + 5, 88);
    ...
    try {
      ...
      // FIX: Use typed endpoint (/image/ or /video/) instead of /auto/
      ...
      // Friendly message for timeout vs network vs server errors
  ...
  const reset = useCallback(() => {
    ...
    dataRef.current = {
      ...
      textOverlayColor: '#ffffff', caption: '',
      previewUrl: null, musicUrl: null, musicTitle: null,
    };
