...
function Draggable({
  children, x, y, onMove,
}: { children: React.ReactNode; x: number; y: number; onMove: (x: number, y: number) => void }) {
...
/* ─── Static canvas (replay strokes without interactivity) ──────────────────── */
...
export default function CreateBlinkPage() {
  ...
  const handleBack = () => {
    ...
    if (step === 'edit') {
      ...
      setActiveEffect('none'); setMusicTitle('');
      setStep('pick'); return;
    }
    reset(); navigate(-1);
  };
      ...
      <AnimatePresence mode="wait">
        ...
        {step === 'pick' && (
          <motion.div key="pick" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col h-full">
            ...
            <div className="flex-1 mx-4 mb-6 rounded-3xl overflow-hidden relative bg-zinc-900 flex flex-col items-center justify-center gap-6">
              ...
              <motion.p initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="text-white/40 text-sm">
        ...
        {step === 'edit' && state.previewUrl && (
          <motion.div key="edit" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="relative h-full w-full">
            ...
            {/* Draw strokes (static display) */}
            {activeTool !== 'draw' && drawStrokes.length > 0 && (
              <StaticCanvas strokes={drawStrokes} />
            )}
            ...
            {musicTitle && activeTool === null && (
              <div className="absolute top-20 left-4 z-10 flex items-center gap-2 bg-black/60 backdrop-blur-md rounded-full px-3 py-1.5 border border-white/10">
                ...
                <button onClick={() => setMusicTitle('')} className="text-white/50 ml-0.5 flex-shrink-0"><X className="w-3 h-3" /></button>
              </div>
            )}
                ...
                <motion.div key="text-tool" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 z-20 flex flex-col">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center px-6 pointer-events-none">
                    <textarea
                      ...
                      style={{ color: textColor, fontSize: textSize, textAlign: textAlign, border: `2px solid ${textColor}55`, caretColor: textColor }}
                    />
                  </div>
                  <motion.div
                    ...
                    <div className="flex gap-2 overflow-x-auto pb-1">
                      {TEXT_COLORS.map(c => (
                        <button key={c} onClick={() => setTextColor(c)}
                          ...
                          style={{ backgroundColor: c, borderColor: textColor === c ? '#fff' : 'transparent', transform: textColor === c ? 'scale(1.18)' : undefined, boxShadow: c === '#FFFFFF' ? 'inset 0 0 0 1px rgba(255,255,255,0.3)' : undefined }} />
                      ))}
                    </div>
            ...
            <AnimatePresence>
              {activeTool === 'music' && (
                <motion.div key="music" initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }}
                  ...
                  <input type="text" value={musicSearch} onChange={e => setMusicSearch(e.target.value)} placeholder="Search for a song..."
                    className="w-full bg-white/10 text-white placeholder:text-white/40 rounded-2xl px-4 py-3 text-sm outline-none border border-white/10 focus:border-white/30 mb-3" />
                  {SUGGESTED_TRACKS.filter(t => !musicSearch || t.toLowerCase().includes(musicSearch.toLowerCase())).map(track => (
                    <button key={track} onClick={() => { setMusicTitle(track); setActiveTool(null); toast.success(`🎵 Added: ${track.split('–')[0].trim()}`); }}
                      className="w-full flex items-center gap-3 px-3 py-3 rounded-2xl hover:bg-white/5 transition-colors text-left">
                      <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: 'linear-gradient(135deg,#667eea,#764ba2)' }}>
            ...
            <div className="relative z-10 flex flex-col h-full px-5 pt-14 overflow-y-auto">
              ...
              <div className="mb-8">
                ...
                <div className="flex gap-3">
                  {['Everyone', 'Following Only'].map((opt, i) => (
                    <button key={opt}
                      className={cn(
                        'flex-1 py-3 rounded-2xl text-sm font-medium transition-all border',
              ...
              {state.uploading && (
                <div className="w-full h-1 bg-white/10 rounded-full overflow-hidden mb-4">
                  <motion.div className="h-full rounded-full" style={{ background: 'linear-gradient(90deg,#667eea,#764ba2)', width: `${state.progress}%` }} transition={{ duration: 0.3 }} />
                </div>
              )}
