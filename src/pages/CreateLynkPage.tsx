...
function ProgressRing({ progress, size = 80 }: { progress: number; size?: number }) {
  ...
  return (
    <svg width={size} height={size} className="rotate-[-90deg]">
      ...
      <circle cx={size/2} cy={size/2} r={r} fill="none" stroke="#FF3B30" strokeWidth={6}
...
export default function CreateLynkPage() {
  ...
  const processFile = useCallback(async (selected: File) => {
    setFileError(null);
    if (!selected.type.startsWith('video/')) { setFileError('Only video files are allowed.'); return; }
    if (selected.size > MAX_FILE_MB * 1024 * 1024) { setFileError(`Max file size is ${MAX_FILE_MB}MB.`); return; }
    try {
  ...
  const reset = () => {
    setFile(null); setThumbnailDataUrl(null); setDuration(0);
    setCaption(''); setCategory('gaming'); setVisibility('public');
    setAllowComments(true); setStep('pick'); setUploadProgress(0);
    setPublishError(null); setFileError(null);
  ...
  return (
    <div className="min-h-screen bg-background text-foreground pb-20">
      <div className="sticky top-0 z-10 bg-background/80 backdrop-blur-md border-b border-border px-4 py-3 flex items-center justify-between">
        ...
        {step === 'details' && (
          <button onClick={reset} className="text-sm text-muted-foreground hover:text-foreground transition-colors">Discard</button>
        )}
      </div>
      ...
      <div className="max-w-lg mx-auto px-4 pt-6 space-y-6">
        ...
        {step === 'pick' && (
          <div
            ...
            className={`flex flex-col items-center justify-center gap-4 cursor-pointer border-2 border-dashed rounded-2xl p-12 transition-all duration-200 ${dragOver ? 'border-primary bg-primary/5 scale-[1.01]' : 'border-border hover:border-primary/50 hover:bg-secondary/50'}`}
          >
            ...
            <div className="text-center">
              ...
              <p className="text-xs text-muted-foreground mt-3">MP4, MOV, WEBM · Max {MAX_FILE_MB}MB · {MIN_DURATION}–{MAX_DURATION}s</p>
            </div>
          ...
          <>
            <div className="flex gap-4 items-start">
              ...
              <div className="flex-1 space-y-1 pt-1">
                ...
                <button onClick={reset} className="text-xs text-primary mt-1 hover:underline">Change video</button>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <textarea value={caption} onChange={(e) => setCaption(e.target.value.slice(0, MAX_CAPTION))}
                  placeholder="Write a caption… add #hashtags to get discovered" rows={3}
                  className="w-full bg-secondary rounded-xl px-4 py-3 text-sm resize-none outline-none focus:ring-2 focus:ring-primary/40 placeholder:text-muted-foreground transition-all" />
              ...
              {hashtags.length > 0 && (
                <div className="flex flex-wrap gap-1.5 mt-1">
                  {hashtags.map(tag => <span key={tag} className="text-xs bg-primary/10 text-primary px-2 py-0.5 rounded-full">#{tag}</span>)}
                </div>
              )}
            </div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {CATEGORIES.map(cat => (
                  <button key={cat.value} onClick={() => setCategory(cat.value)}
                    className={`flex items-center gap-2 px-3 py-2.5 rounded-xl text-sm font-medium border transition-all duration-150 ${category === cat.value ? 'bg-primary text-primary-foreground border-primary' : 'bg-secondary text-foreground border-transparent hover:border-border'}`}>
              ...
              <div className="flex gap-2">
                {(['public', 'unlisted'] as Visibility[]).map(v => (
                  <button key={v} onClick={() => setVisibility(v)}
                    className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-medium border transition-all duration-150 ${visibility === v ? 'bg-primary text-primary-foreground border-primary' : 'bg-secondary text-foreground border-transparent hover:border-border'}`}>
            ...
            <button onClick={handlePost}
              className="w-full bg-primary text-primary-foreground font-semibold py-3.5 rounded-2xl hover:opacity-90 active:scale-[0.98] transition-all duration-150 text-sm">
              Post Lynk 🚀
            </button>
          </>
        )}
        ...
        {step === 'done' && (
          <div className="flex flex-col items-center justify-center gap-6 py-20 text-center">
            ...
            <div className="flex gap-3 w-full">
              <button onClick={reset} className="flex-1 bg-secondary text-foreground font-medium py-3 rounded-2xl hover:bg-secondary/80 transition-colors text-sm">Create Another</button>
              <a href="/lynks" className="flex-1 bg-primary text-primary-foreground font-medium py-3 rounded-2xl hover:opacity-90 transition-opacity text-sm flex items-center justify-center">View Feed</a>
            </div>
          </div>
        )}
